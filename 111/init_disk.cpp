...
#include <vector>
#include<cstring>
#define DISK_SIZE 200*1024*1024*1024
...
#define MAX_DATA_IN_BLOCK 8184 //size_t和long nNextBlock各占4byte
#define MAX_DIR_IN_BLOCK 8
#define MAX_FILENAME 8
#define MAX_EXTENSION 3
long TOTAL_BLOCK_NUM=51200;

#define disk_path "/dev/nvme0n1p2"
...
int create_file_dir(const char* path, int flag)
{
    ...
    char* m = (char*)malloc(15 * sizeof(char)), * n = (char*)malloc(15 * sizeof(char));//用于存放文件名和扩展名
    //拆分路径，找到父级目录起始块
    ...
    struct data_block* data_blk = (struct data_block*)malloc(sizeof(struct data_block));
    struct file_directory* file_dir = (struct file_directory*)malloc(sizeof(struct file_directory));
    int offset = 0;
    int pos;
...
int getoneblk(const char blkpath[1024], off_t offe, char* path) {
    FILE* blk = fopen(blkpath, "r"); //打开块文件
    //long off=ftell(rawsd);
    //fprintf(inforawhd,"start:%ld",off);
    char buf[5024000];
    int len;
    while ((len = fread(buf, sizeof(char), 5024000, blk)) > 0) {
        //fwrite(buf,sizeof(char),1024,rawsd);
        MFS_write(path, buf, len, offe);
        offe += len;
    }
    //offe=ftell(rawsd);
